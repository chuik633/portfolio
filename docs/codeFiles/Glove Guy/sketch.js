...
function buttonInRange(mouseX, mouseY, buttonX, buttonY) {
  if (
    buttonX <= mouseX + buttonWidth / 2 &&
    mouseX + buttonWidth / 2 <= buttonX + buttonWidth
  ) {
    if (
      buttonY <= mouseY + buttonHeight / 2 &&
      mouseY + buttonHeight / 2 <= buttonY + buttonHeight
    ) {
      return true;
    }
  }
...
function createButtons(mouseX, mouseY) {
  ...
  for (const [feature_name, numCols] of Object.entries(num_images)) {
    ...
    text(
      ...
      padding_left - 20,
      padding_top + rowIdx * (buttonHeight + buttonGap)
    );
    ...
    for (let colIdx = 1; colIdx <= numCols; colIdx++) {
      ...
      if (
        ...
        if (feature_name == "legs") {
          ...
          const leg_height = aspectHeight(
            leg_img,
            image_positions["legs"]["width"]
          );
      ...
      image(img, buttonX, buttonY, buttonWidth, aspectHeight(img, buttonWidth));
    }
